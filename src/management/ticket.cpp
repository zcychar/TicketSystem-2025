...
Ticket::Ticket(std::string &name, User *user) : user_(user) {
  ...
  pending_db_ =
      std::make_unique<BPlusTree<TrainDate, PendingInfo, PairCompare<TrainDate>,
                                 PairCompare<TrainDate>>>(name + "_pending_db",
                                                          tdcomp, tdcomp, 128);
  station_db_ =
      std::make_unique<BPlusTree<hash_t, StationTrainInfo, HashComp, HashComp>>(
...
void Ticket::QueryTicket(std::string &from, std::string &to, num_t date,
  ...
  if (comp == "time") {
    ...
    for (auto &train : to_vector) {
      ...
      if (it != train_map.end()) {
        TicketComp ticket{train.arrivingTime -
                              from_vector[it->second].arrivingTime,
                          train.price - from_vector[it->second].price,
                          from_vector[it->second].station_index,
                          train.station_index,
                          from_vector[it->second].leavingTime,
                          train.arrivingTime,
                          train.trainID_hash,
                          {}};
      ...
      while (!queue.empty()) {
        ...
        ticket_db_->GetValue(TrainDate(ticket.trainID_hash, date), &ticketNum);
