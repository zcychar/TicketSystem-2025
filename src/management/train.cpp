...
namespace sjtu {
  ...
  void Train::QueryTrain(std::string &trainID, num_t date) {
    ...
    auto train = train_manager_->ReadPage(train_vector[0].page_id).As<TrainInfo>();
    if (train_vector[0].is_released) {
      ...
      for (int i = 1; i < train->stationNum - 1; ++i) {
        ...
        std::cout << cur_time << ' ' << cur_price << ' ' << ticket_vector[0].seatNum[i] << '\n';
      }
      cur_time += train->travelTimes[train->stationNum - 2];
      cur_price += train->prices[train->stationNum - 2];
      std::cout << train->stations[train->stationNum - 1] << ' ' << cur_time << " -> xx-xx xx:xx " << cur_price << ' '
          << ticket_vector[0].
          seatNum[train->stationNum - 1] << '\n';
    } else {
      DateTime cur_time(date, train->startTime);
      int cur_price = 0;
      std::cout << train->stations[0] << " xx-xx xx:xx -> " << cur_time << ' ' <<
  ...
  auto Train::ReleaseTrain(std::string &trainID) -> bool {
    ...
    StationTrainInfo station_train{train->train_id_hash, {}, 0, train->saleDate, cur_time, cur_time};
